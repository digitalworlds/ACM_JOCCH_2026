...
def prep_data(training_videos, training_labels, validation_videos, validation_labels):
    ...
    # Standardize before oversampling
    ...
    # For each class, randomly sample (with replacement) indices up to max_count
    unique_labels = np.unique(training_labels)
    max_count = max([np.sum(training_labels == label) for label in unique_labels])
    oversampled_indices = np.concatenate([
        np.random.choice(np.where(training_labels == label)[0], size=max_count, replace=True)
        for label in unique_labels
    ])
    
    shuffled_indices = np.random.permutation(oversampled_indices.shape[0])
    training_clips = training_clips[oversampled_indices[shuffled_indices]]
    training_labels = training_labels[oversampled_indices[shuffled_indices]]
